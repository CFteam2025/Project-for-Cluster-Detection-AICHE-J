...
def data_set_split(src_data_folder, target_data_folder, train_scale=0.7, val_scale=0.1, test_scale=0.2):
    ...
    for class_name in class_names:
        ...
        current_all_data = os.listdir(current_class_data_path)   #获取目标类别目录下的所有图像
        current_data_length = len(current_all_data)   #获取当前文件夹内图像数量
        current_data_index_list = list(range(current_data_length))   #获得与图像数量相同的序号列表
        random.shuffle(current_data_index_list)   #进行随机打乱
        # 获取目标目录的文件夹
...
if __name__ == '__main__':
    src_data_folder = r"D:\project_SJ\Resnet50d_classification\datasets_classify\cluster_four_origin.v10"  # todo 修改你的原始数据集路径
    target_data_folder = src_data_folder + "_" + "split712"
