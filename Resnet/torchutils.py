...
def get_torch_transforms(img_size=224):
    ...
    data_transforms = {
        "train": transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.Normalize([0.252, 0.252, 0.252], [0.154, 0.154, 0.154])
        ]),
        "val": transforms.Compose([
            transforms.RandomResizedCrop(img_size, scale=(0.9, 1)),
            transforms.ToTensor(),
            transforms.Normalize([0.252, 0.252, 0.252], [0.154, 0.154, 0.154])
        ])
    }
    ...
    return albumentations.Compose(
        [
            ...
            albumentations.Normalize(
                [0.252, 0.252, 0.252], [0.154, 0.154, 0.154],
...
def calculate_precision_macro(output, target):
    y_pred = torch.softmax(output, dim=1)
    y_pred = torch.argmax(y_pred, dim=1).cpu()
    target = target.cpu()
    # tp fn fp
    return precision_score(target, y_pred, average="macro", zero_division=0)
